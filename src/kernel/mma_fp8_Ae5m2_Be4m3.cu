...
__global__ void mma_fp8_e5m2(const AType * A, const BType * B, float * C, int M,int N, int K) {
    ...
    __shared__ __nv_fp8_e5m2 A_smem[MMA_M][MMA_K];
    ...
    { // 每次循环处理一个子块的乘法
     __syncwarp();
        ...
        *((int4 *)(&A_smem[lane_id / 2][0]) + lane_id % 2) = 
        ...
        // {
        //     printf("A : %f \n", float(A[0]));
        //     // uint8_t byte_value = static_cast<uint8_t>(A[0]);
        //     uint8_t byte_value = *reinterpret_cast<uint8_t*>(const_cast<void*>(static_cast<const void*>(&A_smem[0][0])));
        //     // 打印该 FP8 值的二进制表示
        //     printf("Binary representation of FP8 value: ");
        //     for (int i = 7; i >= 0; --i) {
        //         printf("%d", (byte_value >> i) & 1);  // 提取每一位并打印
        //     }
        //     printf("\n");
        //     printf("B : %f \n", float(B[0]));
        //     uint8_t byte_value_B = *reinterpret_cast<uint8_t*>(const_cast<void*>(static_cast<const void*>(&B_smem[0][0])));
        //     for (int i = 7; i >= 0; --i) {
        //         printf("%d", (byte_value_B >> i) & 1);  // 提取每一位并打印
        //     }
        //     printf("\n");
        // }
        ...
        HMMA16832E5m2(RC[0], RC[1], RC[2], RC[3], RA[0], RA[1], RA[2], RA[3], RB[0], RB[1], RC[0], RC[1], RC[2], RC[3]);   
